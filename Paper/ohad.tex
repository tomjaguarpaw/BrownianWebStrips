...
\text{ or } \twonorm{(x(t), \eeps(t))} <\radius\right)=1$$
Equivalently the two dimensional process $(x(t),\eeps(t))$
...
$(x(t_0), \eeps(t_0))=(0,0)$, let $t>0$ be the first time at which
$\twonorm{(x(t), \eeps(t)}=1$, then the probability
that $(x(t), \eeps(t))=\pm(\frac{1}{\sqrt2},\frac{1}{\sqrt2})$ is at least $1-O(\frac1{\log\epsilon})$.
\end{propos}
...
$x(t)^2+\frac{(\eeps(t)-x(t))^2}{2}=\radiussq$, then the probability
that $x(t)=\eeps(t)=\pm\radius$ is at least
...
$x(s_i)^2+\frac{(\eeps(s_i)-x(s_i))^2}{2}=\radiussq$ then by the definition of
...
$x(s)^2+(\eeps(s)-x(s))^2=\radiussq$ is less then
...
| x(t)^2+(\eeps(t)-x(t))^2=\radiussq, x(t)\neq \radius)<\zeta$
for all $t\in(0,1)$.
\end{proof}
