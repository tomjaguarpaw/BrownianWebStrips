...
Under the previous notations, $e_\epsilon(1) \to x(1)$ in probability
as $\epsilon \to 0$.
\end{theorem}
...
\text{ or }x(t)^2+(e_\epsilon(t)-x(t))^2<\radiussq\right)=1$$
Equivalently the two dimensional process $(x(t),e_\epsilon(t)-x(t))$
...
$x(t)^2+\frac{(e_\epsilon(t)-x(t))^2}{2}=1$, then the probability
...
Let $\radiussq>0$ suppose that at time $t_0$ we have $e(t_0)=(0,0)$, let
$t>0$ be the first time at which
$x(t)^2+\frac{(e_\epsilon(t)-x(t))^2}{2}=\radiussq$, then the probability
that $x(t)=e_\epsilon(t)=\pm\radius$ is at least
...
Let $\radiussq,\zeta>0$ and let $x(t)$ be a one dimensional Brownian
motion such that $x(0)=\radiussq$. There exists some $k$ such that
...
Let $\zeta,\radiussq>0$. We find $\epsilon>0$ such that
$\P(|e_\epsilon(1)- x(1)|\le\radius)<\zeta$. Let $t_1,...,t_k$ be
...
$x(s_i)^2+\frac{(e_\epsilon(s_i)-x(s_i))^2}{2}=\radiussq$ then by the definition of
...
$x(s)^2+(e_\epsilon(s)-x(s))^2=\radiussq$ is less then
...
| x(t)^2+(e_\epsilon(t)-x(t))^2=\radiussq, x(t)\neq \radius)<\zeta$
for all $t\in(0,1)$.
\end{proof}

\subsection{Proof of convergence}\label{sec:POC}
%Let $\xi_\epsilon=(X,Y,\epsilon)$ as before. We denoted by $e_\epsilon(t)$ a sample of $\xi_\epsilon$
