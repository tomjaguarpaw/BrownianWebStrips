...
We define a one-dimensional "$\radius$-Teleporting" Brownian motion: $x^{\radius}(t)$
 as a browning motion such that whenever it hits $0$ it teleports to $\frac1{\sqrt{2}}\radius$.
Proposition \ref{prop:prop1} implies that for all $\zeta>0$ there exists a $k$ such that the probability
 $x^{\radius}(t)$ teleported more then $k$ times between times $0$ and $1$ is less then $\zeta$. Let $x^{\radius}(t)$ be such a teleporting Brownian motion, and let 
...
Let us look at the event that $x^\radius(t)$ have teleported at most $k$ times by time $1$.
 ...
 are independent and by corollary \ref{cor:cor1} can be made as small as we want. However
  if both events do no occur - the coupling insures us that by time $t=1$ the two
