...
\newcommand{\probhitboundaryisnoparams}[1]{the probability that $\twodim$
  hits $\boundarylines$ before it hits $\farpoint$ is #1}
\newcommand{\probhitboundaryis}[1]{For given $P > 0$, $\delta > 0$ \probhitboundaryisnoparams{#1}}
...
\begin{proof}[Proof of Lemma \ref{lem:prob-hit-boundary-o1loge}]
  Before time $T_0$, $\twodim$ is in state $\statewebtogether$ and so
  \probhitboundaryisnoparams{0}.

  After time $T_0$, $\twodim$ consists of a sequence of excursions,
