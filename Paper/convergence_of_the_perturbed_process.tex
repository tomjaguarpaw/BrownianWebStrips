...
Given $\delta>0$, define $\boundarylines=\{(x,y) \ :\  |x-y|=\delta \}$.
...
\begin{proof}[Proof that Lemma \ref{lem:prob-hit-boundary-o1} implies Lemma \ref{lem:resamplede-to-sampled}]

Lemma \ref{lem:resamplede-to-sampled} is equivalent to: For all $\delta > 0$
\[
\P(\twodim_t \in \{(x,y) \ :\  |x-y|<\delta \} \text{ for all } t \in [0,1])\to1 \text{ as } \epsilon\to 0
\]
The reduction of Lemma \ref{lem:resamplede-to-sampled} to the above
\RON{}{How?  Elaborate, since not a Brownian motion}
...
Since $\delta$ is arbitrary, the above statement can now be rephrased as
\[\text{
  The probability that before time $1$, $\twodim$ has hit $\boundarylines$
  is $o(1)$ as $\epsilon \to 0$.
}\]
We can prove this assuming Lemma \ref{lem:prob-hit-boundary-o1}.
...
\RON{}{Need to explain scale invariance here}
