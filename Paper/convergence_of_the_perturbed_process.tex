...
Brownian motion outside the line $\{(x,x)\}$. The transition of $\twodim$ is
...
\begin{claim}\label{cl:tdv-together-estimate}
  Whenever $\rotproc=\origin$ the probability that subsequently $\twodim$
  arrives at $\statewebtogether$ before $\tdv$ hits $\pm\epsilon$ is at
  least a constant (independent of $\epsilon$).
\end{claim}

\begin{claim}\label{cl:tdv-hitting-back-0-estimate}
  Whenever $\tdv=\pm\epsilon$ then there is probability equal to
  $\epsilon/\delta$ of $\tdv$ hitting $\pm\delta$ before it hits $0$.
\end{claim}
...
that between two consecutive times when $\rotproc=\origin$,
\[
...
(see for example Theorem 2.37 on page 52 of \cite{mortens-peres}).
...
\[
\frac{1}{2\pi P} \int_{\epsilon}^{1} \frac{x/\epsilon}{1 + (x/\epsilon)^2}
\, dx
\]
which is
\[
\frac{\epsilon}{4\pi P} \log\left(\frac{1 + (1/\epsilon)^2}{2}\right)
\]
which is $\Omegaeloge$.
\end{proof}
}
